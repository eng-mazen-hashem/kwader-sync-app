-- Migration: 20260804000000_add_deduct_half_on_missing_to_shifts.sql
-- Description: Add deduct_half_on_missing column to shifts table and update process_raw_attendance_trigger to credit half shift hours on missing punch.

ALTER TABLE shifts ADD COLUMN IF NOT EXISTS deduct_half_on_missing boolean DEFAULT false;
...
BEGIN
    ...
    FOR v_punch_ts IN
        ...
        WHERE user_pin = NEW.user_pin
          AND company_id = NEW.company_id
          AND (timestamp - v_offset_interval)::date = v_new_date
