...
BEGIN
    ...
    -- Treat NEW.timestamp as local time before shifting. We convert to UTC first to remove session tz dependency.
    v_new_date := ((NEW.timestamp AT TIME ZONE 'UTC') - v_offset_interval)::date;
    ...
    FOR v_punch_ts IN
        ...
        FROM (
            ...
            WHERE user_pin = NEW.user_pin
              AND company_id = NEW.company_id
              AND ((timestamp AT TIME ZONE 'UTC') - v_offset_interval)::date = v_new_date
              AND (TG_OP = 'INSERT' OR id != NEW.id) -- exclude NEW if it's an UPDATE so we don't double count
