-- Migration: 20260605000001_smart_attendance_processor.sql
-- Description: Overhauls the attendance processing trigger to be fully chronological (Smart Auto-Sorting)
...
BEGIN
    ...
    -- We aggregate ALL punches for this day (already in table + the NEW one)
    -- and recalculate the ENTIRE day based on their exact chronological order.
    ...
    IF v_shift.id IS NOT NULL THEN
        ...
        END IF;
    ELSE
        -- Default start time 08:00 if no shift
        v_start_mins := 8 * 60;
        v_grace_mins := 0;
        v_diff := v_in_mins - v_start_mins;
    ...
    -- Check if today's record already exists in processed_attendance
    ...
    -- If there's only 1 punch, it's just a check-in.
    IF v_punch_count = 1 THEN
        v_last_punch := NULL;
    ELSE
        -- If odd number of punches and deduct_actual, they are currently checked in, so no checkout yet
    ...
    IF v_last_punch IS NOT NULL THEN
        ...
        IF COALESCE(v_shift.break_policy, 'ignore_temp') = 'deduct_actual' THEN
            v_work_hours := ROUND(v_total_seconds::numeric / 3600.0, 2);
        ELSE
            -- ignore_temp or early_leave: total diff between first and last
            v_diff := v_out_mins - v_in_mins;
            IF v_diff < 0 THEN v_diff := v_diff + (24 * 60); END IF;
            IF v_shift.id IS NOT NULL AND v_shift.break_duration IS NOT NULL THEN
                v_diff := v_diff - cast(v_shift.break_duration as int);
