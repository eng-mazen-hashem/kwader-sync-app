-- Migration: 20260805000000_fix_half_shift_recalculation.sql
-- Description: Fix expected hours calculation for half shift deductions by accounting for break duration and ensuring full recalculation.
                ...
                CASE 
                    ...
                    ELSE 
                        ((cast(substring(s.end_time::text from 1 for 2) as int) * 60 + cast(substring(s.end_time::text from 4 for 2) as int)) - 
                         (cast(substring(s.start_time::text from 1 for 2) as int) * 60 + cast(substring(s.start_time::text from 4 for 2) as int))) / 60.0
                END
            ) - (CASE WHEN COALESCE(s.has_break, false) AND s.break_duration IS NOT NULL THEN s.break_duration / 60.0 ELSE 0 END)
        END / 2.0
    )::numeric, 2)
