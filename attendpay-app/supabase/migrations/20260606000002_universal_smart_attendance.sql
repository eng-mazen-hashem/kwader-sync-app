-- Migration: 20260606000002_universal_smart_attendance.sql
-- Description: Introduces Universal Time Offset Equation for all shift types and replaces complex night-shift logic.
...
BEGIN
    ...
    -- Calculate the Time Offset based on shift type
    IF v_shift.id IS NOT NULL THEN
        IF v_shift.shift_type = 'flexible' THEN
            -- Flexible shifts default to a 09:00 AM logical start (cutoff at 05:00 AM)
            v_offset_interval := '05:00:00'::interval;
        ELSE
            -- Fixed shifts logical day starts 4 hours before their exact start time
            v_offset_interval := v_shift.start_time::interval - '04:00:00'::interval;
        END IF;
    ELSE
        -- No shift assigned, assume standard 08:00 AM shift (cutoff at 04:00 AM)
        v_offset_interval := '04:00:00'::interval;
    END IF;

    -- The Magic Equation: Apply the offset to find the exact logical date for this punch
    v_new_date := (NEW.timestamp - v_offset_interval)::date;

    -- ─── 3. CLEANUP PREVIOUS OPEN DAYS ──────────────────────────
    -- Since our offset math automatically grabs punches for the correct day, 
    -- any day strictly BEFORE the calculated v_new_date that lacks a checkout is officially abandoned.
    ...
    -- ─── 4. SMART CHRONOLOGICAL SORTING FOR THE LOGICAL DAY ─────
    -- Define the exact 24-hour window for this specific logical day
    ...
    -- ─── 6. RECALCULATE CHECK-OUT, EARLY LEAVE & WORK HOURS ───
    v_work_hours := 0;
    ...
    IF v_last_punch IS NOT NULL THEN
        ...
        IF NOT v_is_inside_break AND (v_shift.id IS NULL OR v_shift.shift_type != 'flexible') THEN
            ...
            -- Correct wraparound for early leave diff
            IF v_diff < -720 THEN v_diff := v_diff + (24 * 60); END IF;
...
-- Create Universal RPC to safely reprocess ALL employees (not just flexible)
CREATE OR REPLACE FUNCTION reprocess_all_shifts()
RETURNS void AS $$
DECLARE
    emp_record record;
BEGIN
    ...
    LOOP
        ...
        DELETE FROM processed_attendance WHERE employee_id = emp_record.id;
