-- Migration: 20260612030000_add_attendance_conflict_tracking.sql
-- Description: Add conflict tracking and override logic to processed_attendance

ALTER TABLE processed_attendance ADD COLUMN IF NOT EXISTS is_conflicted boolean DEFAULT false;
ALTER TABLE processed_attendance ADD COLUMN IF NOT EXISTS conflict_details jsonb;
ALTER TABLE processed_attendance ADD COLUMN IF NOT EXISTS override_status text DEFAULT 'none'; -- 'none', 'approved_manual', 'approved_device'

-- Ensure we have a constraint or check on override_status
ALTER TABLE processed_attendance DROP CONSTRAINT IF EXISTS processed_attendance_override_status_check;
ALTER TABLE processed_attendance ADD CONSTRAINT processed_attendance_override_status_check CHECK (override_status = ANY (ARRAY['none'::text, 'approved_manual'::text, 'approved_device'::text]));

-- Re-implement the trigger to detect conflicts and handle override logic
...
BEGIN
    ...
    -- Determine if both punch methods exist on this logical day
...
-- Trigger logic is already configured
