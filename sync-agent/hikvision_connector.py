...
class HikvisionConnector:
    ...
    # ─── Request Wrapper (Smart Auth Fallback) ────────────────────────────────
    def _request(self, method: str, path: str, **kwargs) -> requests.Response:
        ...
        # Hikvision firmware fragmentation: Some use Digest (default), some enforce Basic.
        # If Digest fails (401), we automatically fallback to Basic Auth.
        if resp.status_code == 401:
            from requests.auth import HTTPDigestAuth, HTTPBasicAuth
            if isinstance(session.auth, HTTPDigestAuth):
    ...
    def _post(self, path: str, xml_body: str) -> requests.Response:
        ...
        return self._request('POST', path, data=xml_body.encode('utf-8'), headers=headers)
    ...
    def get_attendance_logs(self, since: datetime) -> list[dict]:
        ...
        while True:
            # استخدام hex بدون فواصل (32 حرف) لأن بعض أجهزة Hikvision ترفض الـ UUID العادي
            search_id_str = uuid.uuid4().hex
            try:
